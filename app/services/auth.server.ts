...
const basicAuthPayload = Buffer.from(`${process.env.OAUTH_GITHUB_CLIENT_ID}:${process.env.OAUTH_GITHUB_CLIENT_SECRET}`).toString('base64')
...
authenticator.use(new GitHubStrategy({
    ...
    if (profile.displayName === process.env.GITHUB_USER) return profile.displayName
