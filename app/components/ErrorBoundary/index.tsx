...
export default function ErrorBoundary() {
    ...
    if (isRouteErrorResponse(error)) {
        if (error.status === 503) {
            ...
            const responseData = isDataString ? (
                ...
                <pre className="w-full text-left bg-gray-200 rounded p-2">
        ...
        errorElement = (
            <div className="mb-8">
                ...
                <pre className="px-2 py-1 overflow-y-scroll overflow-hidden max-h-[300px] whitespace-pre-wrap break-all bg-gray-300 rounded">
                    {error.stack}
                </pre>
            </div>
        );
    ...
    return (
        <div className="min-h-dvh flex flex-col items-center justify-center">
            <div className="flex flex-col items-center justify-center max-w-[600px] px-4">
                {errorElement}
                {backButtonElement}
            </div>
        </div>
